...
echo "==> Samba manifest and shared-Projects share"
test -f /etc/samba/shares.d/00-manifest.conf
sudo testparm -s >/dev/null
if command -v smbclient >/dev/null; then
  smbclient -L //127.0.0.1 -U 'alice%alice-file-pass' -m SMB3 2>&1 | grep -q shared-Projects
  smbclient '//127.0.0.1/shared-Projects' -U 'alice%alice-file-pass' -m SMB3 -c ls >/dev/null
  echo "  shared-Projects: accessible"
fi
