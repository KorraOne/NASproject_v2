...
SHARE_PREFIX = "shared-"
MANIFEST_FILENAME = "00-manifest.conf"
...
def rebuild_manifest() -> None:
    """Regenerate include manifest — Samba 4.x does not expand globs in include=."""
    SAMBA_SHARES_STAGING.mkdir(parents=True, exist_ok=True)
    lines = ["# Auto-generated by FrogsWork — lists dynamic share fragments\n"]
    if SAMBA_SHARES_D.is_dir():
        for path in sorted(SAMBA_SHARES_D.glob("*.conf")):
            if path.name == MANIFEST_FILENAME or path.name.startswith("00-"):
                continue
            lines.append(f"include = {path}\n")

    staging = SAMBA_SHARES_STAGING / MANIFEST_FILENAME
    staging.write_text("".join(lines), encoding="utf-8")
    dest = SAMBA_SHARES_D / MANIFEST_FILENAME
